...
export default function ItemProvider({ children }) {
  ...
  const removeItem = (index) => setItems([...items.slice(0, index), ...items.slice(index + 1)]);
  ...
  return (
    <ItemContext.Provider value={{ items, setItems, addItem, removeItem, getTotal, getTotalByUser }}>
      {children}
